...
class BSTMap:               # 이진탐색트리를 이용한 맵
    ...
    def findMax(self): return search_max_bst(self.root, key)

    def findMin(self): return search_min_bst(self.root, key)
    ...
    def insert(self, key, value = None):    # 삽입연산
        ...
        if self.isEmpty():                  # 공백이면
            ...
            insert_bst_iter(self.root, n)   # insert_bst_iter() 호출
...
def inorder(n):                                # 중위 순회 함수
    if n is not None:
        ...
        print(n.key, "->", n.value)            # 루트노드 처리(화면 출력)
...
while True:
    ...
    if s == 'i':
        ...
        name = input('친구의 이름: ')
        if map.search(name) == None:
            print("None")
        else:
            print("친구의 전화번호:", map.search(name).value)
    elif s == 'd':
        name = input('친구의 이름: ')
        map.delete(name)
    elif s == 'l':
